...
const Navbar = () => {
  ...
  return (
    ...
    lg:px-24 xl:px-32 py-4 text-gray-600 border-b border-borderColor
    relative transition-all ${location.pathname ==="/" && "bg-light" }`}>
        ...
        ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            border-borderColor px-3 rounded-full max-w-56'>
                ...
                placeholder-gray-500' placeholder='Search Product'/>
            ...
            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                ...
                <button onClick={() => {user ? logout() : setShowLogin(true)}} className="px-6 py-2 sm:px-8
                 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-all cursor-pointer">{user ? 'Logout' : 'Login'}
                </button>
            </div>
        </div>
